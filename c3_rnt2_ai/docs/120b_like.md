...
- **Banco de expertos LoRA** (muchos adapters especializados, entrenados incrementalmente).
...
pip install -e .
pip install -e .[api]
pip install -e .[hf,train,llama-cpp]   # opcional (según entorno; pip antiguo no acepta llama_cpp)
```
