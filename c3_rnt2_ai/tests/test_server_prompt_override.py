...
def _setup_app(tmp_path: Path, monkeypatch):
    ...
    store.ingest_text("web", "https://example.com", "RAGCTX", quality=0.9)
    settings = {
...
def test_messages_ignore_prompt_and_keep_rag(monkeypatch, tmp_path: Path) -> None:
...
def test_include_sources_returns_refs(monkeypatch, tmp_path: Path) -> None:
    client, _model = _setup_app(tmp_path, monkeypatch)
    resp = client.post(
        "/v1/chat/completions",
        json={"messages": [{"role": "user", "content": "MSG_ONLY"}], "include_sources": True, "max_tokens": 4},
    )
    assert resp.status_code == 200
    payload = resp.json()
